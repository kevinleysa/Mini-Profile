...
folder are mapped to the application's root namespace, 'MiniProfile'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "MiniProfile"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'MiniProfile' in the same way
as `"MiniProfile/sass/src"`.
